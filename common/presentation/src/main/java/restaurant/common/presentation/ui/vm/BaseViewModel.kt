...
open class BaseViewModel(val dm: DataManager)
    ...
    private fun setupRequester(): RxRequester {
        val presentable = object: Presentable {
            override fun showError(error: String) = viewInterface.showErrorInFlashBar(error)

            override fun showError(error: Int) = viewInterface.showErrorInFlashBar(error)

            override fun showLoading() = viewInterface.showLoading()

            override fun hideLoading() = viewInterface.hideLoading()
